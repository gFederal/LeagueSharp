...
namespace FedLeona
{
    class SolarFlare
    {
        ...
        private static readonly float range = 1200;
        private static readonly float rangeSqr = range * range;
        private static readonly float radius = 250;
        ...
        {
            var prediction = Prediction.GetPrediction(target, 0.5f);
        ...
        private static CalculatedPosition GetMultiplePosition(Obj_AI_Hero target, IEnumerable<Obj_AI_Hero> nearTargets)
        {
            foreach (var target2 in nearTargets)
            {
                ...
                Prediction.GetPrediction(target, 0.5f);
                var prediction = Prediction.GetPrediction(target, 0.5f);
                var prediction2 = Prediction.GetPrediction(target2, 0.5f);

                
                if (prediction.Hitchance != HitChance.High || prediction2.Hitchance != HitChance.High)
                    continue;
